...
const columns: ColumnDef<InstituteProps>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="min-w-[5rem]">{row.getValue("instituteName")}</div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="min-w-[5rem]">{row.getValue("instituteAddress")}</div>
    ),
  },
  {
...
export const InstituteTable = () => {
  ...
  const filteredData = useMemo(() => {
    return data
      ...
      .filter((institute) =>
        activeOnly ? institute.status !== "active" : true,
      )
      .slice(0, limit);
